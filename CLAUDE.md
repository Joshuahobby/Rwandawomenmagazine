...
- **Vercel rejects request bodies over 4.5MB** at the platform edge, before Express and before multer — so the upload route's 10MB limit is unreachable in production and a normal phone photo (5-12MB) simply fails. `utils/imageCompression.ts` downscales in the browser first; keep any new upload path behind it.
- **Quill's `getSemanticHTML()` emits `&nbsp;` for every space**, which suppresses line wrapping and overflows narrow columns. `utils/editorContent.ts` undoes it on save. The obvious alternative, `useSemanticHTML={false}`, is worse: raw innerHTML renders bullet lists as `<ol><li data-list="bullet">`, and `data-list` is not in the allowlist, so every bullet list would be sanitized into a numbered one.
- **Inline article images must be uploaded, never embedded:** Quill's stock handlers (toolbar image button, paste, drop) base64 the file into `<img src="data:...">`. The write-side sanitizer's scheme allowlist has no `data:`, so the API rewrote every one to `<img src>` — the image showed while editing and vanished on save. `utils/quillImageUpload.ts` routes all three paths through `/api/media` (Cloudinary) instead, and sweeps any base64 left by a rich-text paste before the article is written. Anything that inserts images into article HTML needs the same treatment.
